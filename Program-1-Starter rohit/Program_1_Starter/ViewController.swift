...
class ViewController: UIViewController {
    ...
    func matchColorLogic() -> Bool{
        ...
        colorNumber = Int(arc4random_uniform(UInt32(self.colorNames.count)))
        while colorNumber == lastButtonAnswer {
            colorNumber = Int(arc4random_uniform(UInt32(self.colorNames.count)))
        }
        lastColorNumber = colorNumber
        
        var randColorbut1 = self.myColors.fetchRandomColor()
        var randColorbut2 = self.myColors.fetchRandomColor()
        var randColorbut3 = self.myColors.fetchRandomColor()
        ...
        // To allow color with big names on a single button.
        btnAnswer1OUTLET.setTitle(randColorbut1.colorName, forState: UIControlState.Normal)
        btnAnswer2OUTLET.setTitle(randColorbut2.colorName, forState: UIControlState.Normal)
        btnAnswer3OUTLET.setTitle(randColorbut3.colorName, forState: UIControlState.Normal)
        
        
        while(buttonAnswer==0||buttonAnswer==1||buttonAnswer==2)
        {
            if(buttonAnswer==0)
            {
                changeColor(randColorbut1.hexValue)
                button1Correct = true;
                button2Correct=false;
                button3Correct=false;
            }
            else if(buttonAnswer==1){
                changeColor(randColorbut2.hexValue)
                button1Correct=false;
                button2Correct = true;
                button3Correct=false;
            }
            else if(buttonAnswer==2)
            {
                changeColor(randColorbut3.hexValue)
                button1Correct=false;
                button2Correct=false;
                button3Correct = true;
            }
